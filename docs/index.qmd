...
Number of people with the respective number of tickets. This will change year-to-year.
...
Creating unique IDs for each individual.
...
Creating the lottery (entrants can have \>1 ticket).
...
Number of entrants: `r length(unique(tickets))` Number of tickets: `r length(tickets)`

Initialize the simulation. These counters for the number of simulations in which a person with that number of tickets is selected.
